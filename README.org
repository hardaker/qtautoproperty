...
*** Available Macros
#+BEGIN_SRC c++
    AUTO_GET(type, varname)            // defines just the varname() function

    AUTO_GET_SIGNAL(type, varname)     // same but all defines a signal
                                       // (but you must emit it!!)

    AUTO_GET_SET_SIGNAL(type, varname) // defines setVarname() too
